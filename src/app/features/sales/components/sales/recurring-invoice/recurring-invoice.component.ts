...
@Component({
  ...
  styleUrls: [
    './recurring-invoice.component.css',
    '../../../../styles/sales-style.css'
  ]
})
export class RecurringInvoiceComponent {
   invoiceNo ='';
  startDate ='';
  invoiceType ='';
  supply ='';
...
editTable(index: number) {
 ...
 filterNumbers(field:  'salePrice' | 'discount' | 'mrp' | 'quantity' | 'payAmount' | 'discountPercent' | 'discountAmount') {
    if (field === 'salePrice') {
...
saveItems() {
  ...
  console.log('Item list:', this.itemList);
   ...
   if (this.itemList.length === 0) {
    this.error = 'Please add at least one item in recurring invoice.';
