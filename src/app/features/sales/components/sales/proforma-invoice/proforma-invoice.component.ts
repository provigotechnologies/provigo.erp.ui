...
@Component({
  ...
  styleUrls: [
    './proforma-invoice.component.css',
...
editTable(index: number) {
 ...
 filterNumbers(field:  'salePrice' | 'discount' | 'mrp' | 'quantity' | 'contactNo' | 'discountPercent' | 'discountAmount') {
    if (field === 'salePrice') {
...
saveItems() {
  ...
  console.log('Item list:', this.itemList);
   ...
   if (this.itemList.length === 0) {
    this.error = 'Please add at least one item in proforma invoice.';
